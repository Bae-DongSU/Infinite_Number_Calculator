...
{ //LINK a와 LINK b를 인자로 받아 결과를 LINK형 자료로 반환하는 함수
    ...
    if (compare(a, b) == 2)
    { //연결리스트 a와 b의 절댓값을 비교해 b의 절댓값이 크다면, a와 b를 스왑한다.
        LINK tmp = a;
        a = b;
        b = tmp;
    }
    ...
    num2 = b->next;
    num2->prev = NULL;
    free(b); //a와 b의 부호를 뗀 숫자값만을 num1과 num2로 가리킨다.
    ...
    a = num2;
    num2 = char_to_list('0');
    concatenate(num2, a); //num1과 num2 합 계산 후 받아올림이 남아있을 경우, 이를 위한 노드를 추가한다. e.g) 500+500
    ...
    while (a != NULL)
    { //a가 NULL 아닐 동안 아래를 반복한다 : 각 자리의 노드를 더해 a에 저장한다.
...
{ //LINK a와 LINK b를 인자로 받아  결과를 LINK형 자료로 반환하는 함수.
    ...
    if (big_num == 3)
    { // 둘의 절대값이 같을 때 둘을 빼면 0이 되므로 결과 반환
        ans = char_to_list('+');
        insert(ans, '.');
        insert(ans, '0');
        free_all(a);
    ...
    for (; b->d != '.'; b = b->next)
        ;
    del_link(b); //소수점이 있는 자리를 카운트하고 소수점을 삭제한다.

    a = last_link(num1);
    b = last_link(num2);

    while (a != NULL)
    { //a가 NULL 아닐 동안 아래를 반복한다 : 각 자리의 노드를 빼 a에 저장한다.
        char number1 = a->d - '0';
        char number2 = b->d - '0';
        char check = 0;
...
{ //LINK a와 LINK b를 인자로 받아 곱한 결과를 LINK형 자료로 반환하는 함수
    ...
    num_copy = num2_last;
    for (; num_copy->d != '.'; num_copy = num_copy->prev)
        point++; //3+4=7  //num1, num2의 소수점 아래 자릿수를 카운트한다.
    ...
    for (; num_copy->d != '.'; num_copy = num_copy->next)
        ;
    del_link(num_copy); //num1, num2의 소수점을 삭제한다.

    mul1 = last_link(num1_int);
    unsigned long long int zero = 0;
    for (; mul1 != NULL; mul1 = mul1->prev)
    { //mul1이 NULL이 아닐 때동안 아래를 반복한다. : 각 자리 노드를 곱하고, 그 값을 전의 결과값과 더한다.
        ...
        { //mul2가 NULL이 아닐때 동안 아래를 반복한다. : mul2 연결리스트 수식과 mul1의 노드를 곱하는 연산을 한다.
            if (mul2->d == '.')
            {
                continue;
            }
        ...
        if (!zero)
            ans = copy_link(input); //zero가 0이면 input의 연결리스트를 복사하여, ans로 가리킨다

        else
        { //zero가 0이 아닐 때 아래를 수행한다. : 곱하여 나온 값을 전에 나온 값과
    ...
    if (anslen > point)
    { //결과값의 길이가 point 크기보다 더 클 때 (e.g 12.345)
        num_copy = ans;
        save = ans;
        if (positive)
            ans = char_to_list('+');
    ...
    { //결과값의 길이가 point 크기보다 작을 때 (e.g 0.001)
        num_copy = ans;
        save = ans;
